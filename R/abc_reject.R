...
abc_reject <- function(parameters = processed_params, lines = lines, summary_stat_target = 0, tol = 1) {

  parameters_reject <- lapply(X = parameters, FUN = function(x) { abc::abc(target = summary_stat_target, param = x[,2:ncol(x)], sumstat = x[,"stats", drop = FALSE], tol = tol, method = "rejection")})
